...
describe('transactional installation', () => {
  ...
  it('backs up existing installation and preserves unrelated user files', async () => {
    ...
    await commitInstall(game, stage, 'r1');
    expect(await fs.readFile(path.join(game, 'BepInEx/plugins/my-mod.dll'), 'utf8')).toBe(
      'personal',
    );
    expect(await fs.readFile(path.join(game, 'winhttp.dll'), 'utf8')).toBe('new-loader');
    const state = path.join(game, '.vlauncher');
  ...
  it('removes only obsolete managed files on the next release', async () => {
    ...
    expect(await exists(path.join(game, 'BepInEx/plugins/personal.dll'))).toBe(true);
  });
