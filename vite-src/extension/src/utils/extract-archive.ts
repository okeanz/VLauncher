...
export const extractArchive = async (archivePath: string, extractPath: string): Promise<void> => {
  try {
    ...
    logInfo(`[extractArchive] Successfully extracted ${extractedCount} files from ${archivePath} to ${extractPath}`);
  } catch (e) {
    logError(`[extractArchive] Error extracting ${archivePath}: ${e instanceof Error ? e.message : String(e)}`);
    throw e;
  }
};
